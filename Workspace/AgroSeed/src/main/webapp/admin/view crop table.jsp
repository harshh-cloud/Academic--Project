...
<table class="myTable" style="padding-bottom:30px;" >
 ...
 //	int i =1;
	...
	<!--   <td><//%= i%></td> --> 
		...
		<td><a href="Updatecrop.jsp?sn1= <%= rs.getString("Crop") %>&sn2=<%=rs.getString("Description") %>&sn3=<%=rs.getString("Fertilizers") %>&sn4=<%=rs.getString("Water") %>&sn5= <%= rs.getString("Pesticides") %> ">Update</a></td>
 	<td><a href="Deletecrop.jsp?sn= <%= rs.getString("Id") %>">Delete</a></td>
		</tr>
	<%// i++;
	}
	%>	
